...
/// Ordered filters and a default automatic-tiling decision.
struct WindowRule: Encodable {
  ...
  static func parse(arguments: [String]) throws -> WindowRule {
    ...
    for argument in arguments {
      ...
      guard ["label", "app", "app!", "title", "title!", "bundle-id", "manage"].contains(key),
        !value.isEmpty
    ...
    guard let manage = values["manage"], ["on", "off"].contains(manage) else {
      throw IPCCommandError.invalidRequest("rule requires manage=on|off")
    }
    if let label = values["label"], Int(label) != nil {
      throw IPCCommandError.invalidRequest("rule label must not be an integer")
    }
    return try WindowRule(values: values, manage: manage == "on")
  }
  ...
  private init(values: [String: String], manage: Bool) throws {
    self.values = values
